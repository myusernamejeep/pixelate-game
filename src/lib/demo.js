...
Filter.prototype.use = function() {
    ...
    var image = images[this.imageFile || defaultImg];
    ...
    var tbody = $('#properties')[0].firstChild.nextSibling;
    for (var tr = tbody.firstChild.nextSibling; tr; tr = next) {
...
var images = {
    'dummy.jpg': { credit: 'matthigh', url: 'http://www.flickr.com/photos/matthigh/2125630879/' },
    'lighthouse2.jpg': { credit: 'renet', url: 'http://www.flickr.com/photos/renet/12135813/' },
...
var adjustSizeForWindow = function() {
    var block = $('.block')[0];
    var canvas = $('#canvas_img')[0];//.css({ width: texture._.width, height: texture._.height });
    var properties = $('#properties')[0];
    //console.log(canvas);
    if (navigator.userAgent.match(/iPhone/i) || navigator.userAgent.match(/iPod/i)) {
                var a = document.documentElement.clientWidth - document.body.clientWidth;
                cc.canvas.width = document.documentElement.clientWidth < cc.originalCanvasSize.width ? cc.originalCanvasSize.width : document.documentElement.clientWidth - a;
                cc.canvas.height = document.documentElement.clientHeight < cc.originalCanvasSize.height ? cc.originalCanvasSize.height : document.documentElement.clientHeight - a
    } else cc.canvas.width = document.documentElement.clientWidth, cc.canvas.height = document.documentElement.clientHeight;
    ...
    if (b = canvas) b.style.width = cc.canvas.width + "px", b.style.height = cc.canvas.height + "px";
    if (b = block) b.style.width = cc.canvas.width + "px", b.style.height = cc.canvas.height + "px";
    if (b = properties) b.style.width = cc.canvas.width + "px" ;
    
    //var ctx = document.getElementById('canvas_img').getContext('2d');
    //ctx.translate(0, cc.canvas.height);
    //ctx.scale(a, a);
    //canvas.renderContext.translate(0, canvas.height);
    //canvas.renderContext.scale(a, a);
    //cc.Director.getInstance().setContentScaleFactor(a);
    ///Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent) & document.documentElement.clientWidth > document.documentElement.clientHeight ? (document.getElementById("orientation").style.display = "", document.getElementById("Cocos2dGameContainer").style.display = "none") : (document.getElementById("orientation").style.display = "none", document.getElementById("Cocos2dGameContainer").style.display = "");
    //(navigator.userAgent.match(/iPhone/i) || navigator.userAgent.match(/iPod/i) || navigator.userAgent.match(/iPad/i)) && window.scrollTo(0, 55)
}
...
var filters = {
    ...
    'Blur': [
        ...
        new Filter('Tilt Shift', 'tiltShift', function() {
            this.addNub('start', 0.15, 0.25);
            this.addNub('end', 0.35, 0.45);
    ...
    'Fun': [
        ...
        new Filter('Dot Screen', 'dotScreen', function() {
            ...
            this.addSlider('size', 'Size', 1, 15, 3, 0.01);
        }, function() {
